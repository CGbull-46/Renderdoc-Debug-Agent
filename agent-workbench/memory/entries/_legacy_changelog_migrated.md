...
### Added
- `config/models.json` 作为 Planner/Action 模型枚举配置源。
- Orchestrator 项目工作区与 `/projects/*` 接口（resources/history）。
### Changed
- Sidebar 接入本地 Projects/Resources 管理与导入/创建流程。
- Settings 仅保留 OpenRouter API Key、Planner Model、Action Model，并从 `/models` 拉取枚举。
- Orchestrator HTTP 默认仅监听 `127.0.0.1`。

## 2025-12-31 - Agent Workbench 目录迁移（原 Coding Agent Intermediate）
### Changed
- 将 coding agent 的中间产物（增量 changelog、plans）从 `docs/agent/` 迁移到仓库根目录 `agent-workbench/`，避免与项目功能上下文工程混放。
### Notes
- 新路径：`agent-workbench/changelog.md`、`agent-workbench/plans/*`

## 2025-12-31 - Memory 详细记录模板
### Added
- `agent-workbench/memory/entries/_template.md`：每次迭代详细记录模板（含元数据、Working Set、文档同步与验收清单），用于降低记录格式漂移与后期检索成本。
