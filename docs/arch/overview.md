...
- **本地 Agent**（`runtime/agent/`）：用 Python 封装 RenderDoc API，暴露为 MCP 工具，确保调试调用具有确定性和可重放性。
...
2. Planner 模型根据 `runtime.agent.runtime.RenderdocDebugAgent.list_mcp_tools()` 提供的 JSON Schema 规划工具调用序列。
...
- `runtime.agent.renderdoc_adapter.load_renderdoc()` 自动搜寻 Windows/macOS/Linux 常见的 Python 绑定路径，可通过 `RENDERDOC_PYTHON_PATH` 强制指定。
